      ...
      <div className="flex-1 justify-start sm:px-4 p-12 sm:pb-4 ">
        ...
        <div className="flex flex-row sm:flex-col mt-10">
          {currentAccount === nft.seller.toLowerCase() ? (
            ...
            ? (<Button btnName="List on Marketplace" classStyles="rounded-xl mr-5 sm:mr-0 mb-5" handleClick={() => router.push(`/resell-nft?tokenId=${nft.tokenId}&tokenURI=${nft.tokenURI}`)} />)
            : (<Button btnName={`Buy for ${nft.price} ${nftCurrency}`} classStyles="rounded-xl mr-5 sm:mr-0 mb-5" handleClick={() => { setPaymentModal(true); }} />)}
        </div>
      </div>
      {paymentModal && (
      <Modal
